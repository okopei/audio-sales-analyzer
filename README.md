...
| システム | 問題 | 発生状況 | 解決方法 |
|---------|------|---------|---------|
| Azure Blob Storage | コンテナへのアクセスエラー（404） | フィードバック画面で音声ファイルの読み込みに失敗 | 1. Azure Portalで新しいSASトークンを生成<br>2. 必要な権限を設定（Read, List）<br>3. .env.localのNEXT_PUBLIC_AZURE_STORAGE_SAS_TOKENを更新<br>4. アプリケーションを再起動 |
